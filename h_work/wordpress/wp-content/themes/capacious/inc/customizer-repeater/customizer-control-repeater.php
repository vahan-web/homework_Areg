...
if (class_exists('WP_Customize_control')) {
    ...
    class Capacious_Repeater_Controler extends WP_Customize_Control
    {
        /**
         * The control type.
         *
        ...
        public $capacious_box_label = '';

        public $capacious_box_add_control = '';
        ...
        {
            $this->fields = $fields;
            $this->capacious_box_label = $args['capacious_box_label'];
            $this->capacious_box_add_control = $args['capacious_box_add_control'];
        ...
        <?php } ?>
            ...
            <ul class="capacious-repeater-field-control-wrap">
                <?php
                $this->capacious_get_fields();
                ?>
            </ul>

            <input type="hidden" <?php $this->link(); ?> class="capacious-repeater-collector"
                   value="<?php echo esc_attr($this->value()); ?>"/>
            <button type="button"
                    class="button capacious-add-control-field"><?php echo esc_html($this->capacious_box_add_control); ?></button>
            <?php
        }

        private function capacious_get_fields()
        {
            $fields = $this->fields;
            $values = json_decode($this->value());
            ...
            if (is_array($values)) {
                foreach ($values as $value) {
                    ?>
                    <li class="capacious-repeater-field-control">
                        <h3 class="capacious-repeater-field-title"><?php echo esc_html($this->capacious_box_label); ?></h3>

                        <div class="capacious-repeater-fields">
                            <?php
                            foreach ($fields as $key => $field) {
                                $class = isset($field['class']) ? $field['class'] : '';
                                ?>
                                <div
                                        class="capacious-fields capacious-type-<?php echo esc_attr($field['type']) . ' ' . $class ?>">
                                    ...
                                    switch ($field['type']) {
                                        ...
                                        case 'upload':
                                            ...
                                            echo '<div class="capacious-fields-wrap">';
                                            echo '<div class="attachment-media-view">';
                                            echo '<div class="placeholder' . $image_class . '">';
                                            esc_html__('No image selected', 'capacious');
                                            echo '</div>';
                                            echo '<div class="thumbnail thumbnail-image">';
                                            echo $image;
                                            echo '</div>';
                                            echo '<div class="actions clearfix">';
                                            echo '<button type="button" class="button capacious-delete-button align-left">' . esc_html__('Remove', 'capacious') . '</button>';
                                            echo '<button type="button" class="button capacious-upload-button alignright">' . esc_html__('Select Image', 'capacious') . '</button>';
                                        ...
                                        case 'category':
                                            ...
                                            echo '<option value="0">' . esc_html__('Select Category', 'capacious') . '</option>';
                                            echo '<option value="-1">' . esc_html__('Latest Posts', 'capacious') . '</option>';
                                        ...
                                        case 'colorpicker':
                                            echo '<input data-default="' . esc_attr($default) . '" class="capacious-color-picker" data-alpha="true" data-name="' . esc_attr($key) . '" type="text" value="' . esc_attr($new_value) . '"/>';
                                            break;
                                        ...
                                        case 'range':
                                            ...
                                            echo '<div class="capacious-range-slider" >';
                                        ...
                                        case 'icon':
                                            echo '<div class="capacious-selected-icon">';
                                            echo '<i class="' . esc_attr($new_value) . '"></i>';
                                            echo '<span><i class="fa fa-angle-down"></i></span>';
                                            echo '</div>';
                                            echo '<ul class="capacious-icon-list clearfix">';
                                            $capacious_font_awesome_icon_array = capacious_font_awesome_icon_array();
                                            foreach ($capacious_font_awesome_icon_array as $capacious_font_awesome_icon) {
                                                $icon_class = $new_value == $capacious_font_awesome_icon ? 'icon-active' : '';
                                                echo '<li class=' . $icon_class . '><i class="' . $capacious_font_awesome_icon . '"></i></li>';
                                            }
                                            echo '</ul>';
                                            echo '<input data-default="' . esc_attr($default) . '" type="hidden" value="' . esc_attr($new_value) . '" data-name="' . esc_attr($key) . '"/>';
                                            break;
                                        ...
                                        case 'multicategory':
                                            ...
                                            echo '<ul class="capacious-multi-category-list">';
                                            echo '<li><label><input type="checkbox" value="-1" ' . checked('-1', $new_value, false) . '/>' . esc_html_e('Latest Posts', 'capacious') . '</label></li>';
                                            foreach ($this->cats as $cat) {
                            ...
                            <div class="clearfix capacious-repeater-footer">
                                <div class="alignright">
                                    <a class="capacious-repeater-field-remove"
                                       href="#remove"><?php esc_html_e('Delete', 'capacious') ?></a> |
                                    <a class="capacious-repeater-field-close"
                                       href="#close"><?php esc_html_e('Close', 'capacious') ?></a>
                                </div>
                            </div>
                        </div>
                    </li>
...
if (!function_exists('capacious_font_awesome_icon_array')) {
    function capacious_font_awesome_icon_array()
    {
