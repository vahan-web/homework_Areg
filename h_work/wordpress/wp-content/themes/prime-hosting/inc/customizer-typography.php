 ...
 */
function prime_hosting_customize_register_typography( $wp_customize ) {
	$wp_customize->add_panel(
		'typography_panel',
		array(
			'title'    => __( 'Typography', 'prime-hosting' ),
			'priority' => 50,
		)
	);
	...
	$wp_customize->add_section(
		'body_and_content',
		array(
			'title'    => __( 'Body & content', 'prime-hosting' ),
	...
	$wp_customize->add_setting(
		...
		array(
			'default'           => 'Roboto',
			'sanitize_callback' => 'prime_hosting_sanitize_select',
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			...
			'label'   => __( 'Select a font family', 'prime-hosting' ),
			'choices' => prime_hosting_fonts_list(),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'        => 'number',
			'section'     => 'body_and_content',
			'label'       => __( 'Select a base font-size (px) for the body', 'prime-hosting' ),
			'input_attrs' => array(
				'min'  => 14,
				'max'  => 24,
				'step' => 1,
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'select',
			'section' => 'body_and_content',
			'label'   => __( 'Select a font weight', 'prime-hosting' ),
			'choices' => array(
				'400' => __( '400 Normal', 'prime-hosting' ),
				'700' => __( '700 Bold', 'prime-hosting' ),
			),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'select',
			'section' => 'body_and_content',
			'label'   => __( 'Select a default line-height for the body', 'prime-hosting' ),
			'choices' => array(
				'1'   => __( '1', 'prime-hosting' ),
				'1.5' => __( '1.5', 'prime-hosting' ),
				'2'   => __( '2', 'prime-hosting' ),
				'2.5' => __( '2.5', 'prime-hosting' ),
				'3'   => __( '3', 'prime-hosting' ),
				'3.5' => __( '3.5', 'prime-hosting' ),
			),
		)
	);
	...
	for ( $i = 1; $i < 7; $i++ ) {
		...
		$wp_customize->add_section(
			'heading_' . $i,
			array(
				'title'    => 'H' . $i . ' ' . __( 'Heading', 'prime-hosting' ),
				'priority' => 80,
				'panel'    => 'typography_panel',
			)
		);
		...
		$wp_customize->add_control(
			...
			array(
				'type'        => 'number',
				'section'     => 'heading_' . $i,
				'label'       => 'H' . $i . ' ' . __( 'Font size', 'prime-hosting' ),
				'description' => __( 'Select a font size for the heading (px).', 'prime-hosting' ),
				'input_attrs' => array(
					'min'  => 20,
					'max'  => 94,
					'step' => 1,
		...
		$wp_customize->add_control(
			...
			array(
				'type'    => 'select',
				'section' => 'heading_' . $i,
				'label'   => 'H' . $i . ' ' . __( 'Select a font weight', 'prime-hosting' ),
				'choices' => array(
					'400' => __( '400 Normal', 'prime-hosting' ),
					'700' => __( '700 Bold', 'prime-hosting' ),
				),
		...
		$wp_customize->add_control(
			...
			array(
				'type'        => 'select',
				'section'     => 'heading_' . $i,
				'label'       => 'H' . $i . ' ' . __( 'Capitalization', 'prime-hosting' ),
				'description' => __( 'You can choose to show the text as all uppercase or capitilized.', 'prime-hosting' ),
				'choices'     => array(
					'initial'    => __( 'Default', 'prime-hosting' ),
					'uppercase'  => __( 'Uppercase', 'prime-hosting' ),
					'capitalize' => __( 'Capitalized', 'prime-hosting' ),
				),
			)
		);
		...
		$wp_customize->add_control(
			...
			array(
				'type'    => 'select',
				'section' => 'heading_' . $i,
				'label'   => 'H' . $i . ' ' . __( 'Line height', 'prime-hosting' ),
				'choices' => array(
					'1'   => __( '1', 'prime-hosting' ),
					'1.5' => __( '1.5', 'prime-hosting' ),
					'2'   => __( '2', 'prime-hosting' ),
...
add_action( 'customize_register', 'prime_hosting_customize_register_typography' );

/**
 * Add custom typography styles.
 */
function prime_hosting_typography_css() {
	echo '<style id="prime_hosting-typography-css">';
	...
	if ( get_theme_mod( 'body_font_weight', '400' ) !== '400' ) {
		echo 'font-weight:' . esc_html( get_theme_mod( 'body_font_weight', '400' ) ) . ';';
	}

	echo '}';

	if ( get_theme_mod( 'body_font_family', 'Roboto' ) !== 'Roboto' ) {
		echo 'body { font-family: ' . esc_html( get_theme_mod( 'body_font_family', 'Roboto' ) ) . ';}';
	}
	...
	for ( $i = 1; $i < 7; $i++ ) {
		...
		if ( get_theme_mod( 'heading_ ' . $i . '_font_family', 'Montserrat' ) !== 'Montserrat' ) {
			echo 'font-family:' . esc_html( get_theme_mod( 'heading_ ' . $i . '_font_family', 'Montserrat' ) ) . '!important; ';
		}
...
add_action( 'wp_head', 'prime_hosting_typography_css' );

/**
 * Enqueue the selected fonts.
 */
function prime_hosting_custom_fonts() {

	$body_font = get_theme_mod( 'body_font_family', 'Roboto' );
	if ( ! empty( $body_font ) && $body_font !== 'Roboto' ) {
		$body_font = '//fonts.googleapis.com/css2?family=' . $body_font . ':wght@200;300;400;500;600;700&&	display=swap';
		wp_enqueue_style( 'prime-hosting-body-font', $body_font, array(), null );
	}
	...
	if ( ! empty( $heading_fonts_final ) ) {
		wp_enqueue_style( 'prime-hosting-heading-fonts', '//fonts.googleapis.com/css2?' . $heading_fonts_final . 'display=swap', array(), null );
	}
}

add_action( 'wp_enqueue_scripts', 'prime_hosting_custom_fonts' );
